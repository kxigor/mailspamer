						...
						char **argv)
{
	...
	if(argc == 2){
		...
		if(str == "-h" || str == "--help"){
			...
			cout << " Name of the history file \""+name_file_history+"\""	<< endl;
			exit(0);
		}
	}
	if(argc == 7){
...
int main(int argc, char *argv[])
{
	...
	arguments_function(&email_attack,&content,&num_cy,&num_threads,argc,argv);
	
	vector <account> emails;
	
	get_mail_from_txt(&emails,name_file_emails);
	
	std::cout << " " << emails.size() << " - GOOGLE MAIL WAS FOUND  " << endl;
	
	if(!emails.size()) v_error( " There are no emails ! \
								\n Add emails in the format gm@gmail.com:pass to file emails\
	...
	if(content.name == "")
	{
		...
		getline ( std::cin,content.name);
		
		cout << " Enter the subject of the letter (r/s) : ";
		getline ( std::cin,content.subject);
	
		cout << " Enter \"fr\" to use the text from the file" << endl;
		cout << " Enter the text of the letter (r/s/fr) : ";
		getline ( std::cin,content.body);
	}
	...
	if (num_cy < 1)	v_error(" Invalid input data ",error_fatal);
